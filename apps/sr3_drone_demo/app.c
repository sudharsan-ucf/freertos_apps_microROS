...
rcl_subscription_t subscriber_imu;
...
{
	const sensor_msgs__msg__Imu * msgPtr = (const sensor_msgs__msg__Imu *)msgin;

	// msg->linear_acceleration.x
	RCSOFTCHECK(rcl_publish(&publisher_imu, msgPtr, NULL));
}
...
void appMain(void * argument)
{
	...
	RCCHECK(rclc_subscription_init_default(
		&subscriber_imu, &node, ROSIDL_GET_MSG_TYPE_SUPPORT(sensor_msgs, msg, Imu),
		"/demo/imu0"));

	// create executor
	rclc_executor_t executor;
	RCCHECK(rclc_executor_init(&executor, &support.context, 1, &allocator));
	RCCHECK(rclc_executor_add_subscription(&executor, &subscriber_imu, &msg_imu, &subscription_callback, ON_NEW_DATA));
